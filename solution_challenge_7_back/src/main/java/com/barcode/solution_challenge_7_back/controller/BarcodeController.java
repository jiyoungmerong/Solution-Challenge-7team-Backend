...
import com.barcode.solution_challenge_7_back.service.RecyclingInfoService;
...
public class BarcodeController {
    ...
    private final RecyclingInfoService recyclingInfoService;
...
//    }
    ...
    @ApiOperation(value = "분리수거 방법", notes = "물질에 따른 분리수거 방법을 조회합니다.")
    @GetMapping("/{material}")
    public ResponseEntity<String> getRecyclingInfo(@PathVariable String material) {
        String recyclingInfo = recyclingInfoService.getRecyclingInfoByMaterial(material);
        if (recyclingInfo != null) {
            return ResponseEntity.ok(recyclingInfo);
        } else {
            return ResponseEntity.notFound().build();
